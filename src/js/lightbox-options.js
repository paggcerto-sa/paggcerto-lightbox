...
class LightboxOptions {
  ...
  _validate() {
    ...
    this._options.payment.payers.forEach((payer, index) => {
      ...
      if (!_isNullOrUndefined(payer.mobile) && !this._isString(payer.mobile)) throw new Error(`Invalid options.payment.payers[${index}].mobile: Expected String.`)
      if (!_isNullOrUndefined(payer.email) && !this._isString(payer.email)) throw new Error(`Invalid options.payment.payers[${index}].mobile: Expected String.`)
    })
